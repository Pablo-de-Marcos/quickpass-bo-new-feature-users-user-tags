...
function GeneralSection() {
  ...
  const EditEvent = async (response: ValuesFormProps, setSubmitting, resetForm, setStatus, setErrors) => {
    ...
    handleSnackBar('Evento editado con éxito', 'success');
    resetForm();
    setStatus({ success: true });
    setSubmitting(false);
      ...
      <Grid container spacing={3}>
        <Formik
          ...
          onSubmit={async( _values, { resetForm, setErrors, setStatus, setSubmitting }) => {
            try {
              if (thumbnail.file) {
                EditEvent(_values, setSubmitting, resetForm, setStatus, setErrors);
